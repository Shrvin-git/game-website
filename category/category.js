...
const categorisWrapper = document.querySelector('.categoris-wrapper')
...
(async () => {
  ...
  const fetchingAllDatas = await fetch(supabaseURL, {
    method: "GET",
    headers: {
    ...
    .then(res => res.json())
    .then(data => filteredGamesByCategories(data))
})()


const filteredGamesByCategories = (allGames) => {

  genreName.innerHTML = `Searching For ( ${category} ) Games `

  const filtredGames = allGames.filter(games => games.genre === category)

  if (filtredGames.length === 0) {
    categorisWrapper.insertAdjacentHTML('beforeend',
      `
      <div class="filter_game" id="filter_game">
         <div id="search-result__wrapper">
               <h3 id="search-result"> No game found ! </h3>
         </div>
      </div>
      `
    )
  } else {
    filtredGames.forEach(element => {
      categorisWrapper.insertAdjacentHTML('beforeend',
        `
            <div class="filter_game_item">
              ...
              <h3 class="filter_game_name">${element.title}</h3>
              <div class="filter_game_info">
                <div>
                  ...
                  <span class="relese-games-filtred">${element.release.slice(0, 4)}</span>
                </div>
